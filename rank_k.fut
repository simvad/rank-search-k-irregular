...
let split [n][m] (d_as:[n]i64) (s_as:[m]i64) (rng:rng_engine.rng): ([n]i64,[3][n]i64, [m][3]i64, rng_engine.rng, [m]i64) =
    ...
    let cs_cumm= (map (sgmscan (+) 0 f_as) cs)
    ...
    in (f_as,cs,splits,rng,b_as)

let merge [n] [m] (d_as:[n]i64) (s_as: [m]i64) (f_as:[n]i64) (b_as:[m]i64) (all:[3][n]i64) (cls:[3][n]i64) =
    ...
    let sub_offs = sgmscan (+) 0 f_as remains
    let iss = scatter (replicate n (-1)) (map (\x->x-1)(map2 (+) b_as s_as)) ((0...m-1):>[m]i64) :>[n]i64
    let s_res = scatter (replicate m 0) iss sub_offs
    in (s_res,d_res)

let rank_k [l] (s_as: [l]i64) (d_as: []i64) (ks: [l]i64) =
    let rng = rng_engine.rng_from_seed [123]
    let stop = (reduce (+) 0 s_as) <= length s_as
    let (_, d_res,_,_,_) =
        loop(s_as, d_as,ks,rng, stop) while (!stop) do
            let tmp_split = split d_as s_as rng
            let f_as = tmp_split.0
            let cls = tmp_split.1
            let splits = tmp_split.2  
            let rng = tmp_split.3
            let b_as = tmp_split.4
            ...
            let tmp_merge = merge d_as s_as f_as b_as all cls
            let s_res = tmp_merge.0
            let d_res = tmp_merge.1
            let stop = (reduce (+) 0 s_res)<=(length s_res)
            in (s_res, d_res,ks_new, rng, stop)
    in d_res     

let main (ks: []i64) =
    let d_as = [1,5,7,2,3,1,0,2,3,0,1,9,4,8,12,14,11,9,2,1,5,6,8] :>[23]i64
    let s_as = [7,5,4,7]:>[4]i64
    in rank_k s_as d_as ks
